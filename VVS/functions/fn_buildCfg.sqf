...
// FOR EXAMPLE, YOU WANT TO ADD CLASS "AIR2" TO AIR VEHICLE SPAWNER. ADD IT ON LINE 83 RIGHT AFTER "rhs_vehclass_helicopter".
		...
		// CARS and trucks
					if ((count VVS_Car == 0) && (_vehicleClass in ["Car","rhs_vehclass_car","rhs_vehclass_truck","RSR_vehicleClass_flecktarn","RSR_vehicleClass_tropentarn"])) then
		...
		// AIR
					if ((count VVS_Air == 0) && (_vehicleClass in ["Air","rhs_vehclass_aircraft","rhs_vehclass_helicopter","RSR_vehicleClass_flecktarn","RSR_vehicleClass_tropentarn"])) then
					{ VVS_pre_Air set[count VVS_pre_Air,_className]; };
		// ARMORED (TANKS, APC, IFV, ARTILLERY)
					if ((count VVS_Armored == 0) && (_vehicleClass in ["Armored","rhs_vehclass_tank","rhs_vehclass_apc","rhs_vehclass_ifv","rhs_vehclass_artillery","RSR_vehicleClass_flecktarn","RSR_vehicleClass_tropentarn"])) then
					{ VVS_pre_Armored set[count VVS_pre_Armored,_className]; };
		// Autonomous and rhs AA
					if ((count VVS_Autonomous == 0) && (_vehicleClass in ["Autonomous","rhs_vehclass_aa","RSR_vehicleClass_flecktarn","RSR_vehicleClass_tropentarn"])) then
