...
if(_parameterCorrect && hasInterface) then {
    ...
    private _actionCrateMortar = [ACTION_CRATE_AMMO_ID, ACTION_CRATE_AMMO_NAME, "", {
        params ["_target","_caller"];
        private _crate = CRATE_AMMO_CLASS createVehicle getPos _target;
        clearMagazineCargoGlobal _crate;
        _crate addItemCargoGlobal ["ACE_1Rnd_82mm_Mo_HE",20];
        _crate addItemCargoGlobal ["ACE_1Rnd_82mm_Mo_Smoke",6];
        _crate addItemCargoGlobal ["ACE_1Rnd_82mm_Mo_Illum",6];
    }, {true}] call ace_interact_menu_fnc_createAction;
    [_object,0,["ACE_MainActions"], _actionCrateMortar] call ace_interact_menu_fnc_addActionToObject;
    // delete crates
