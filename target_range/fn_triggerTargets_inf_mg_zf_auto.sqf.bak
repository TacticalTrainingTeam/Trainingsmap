...
for "_i" from 1 to 5 do
{
    ...
    [_target,_i,_object] spawn
    {
        ...
        //if !(_object  getVariable ["target_range_isUp_mg_zf", false]) exitWith {};
        if (_object  getVariable ["target_range_abort_mg_zf", false]) exitWith {};
        ...
        if (_timeout) then
        {
            ...
            if (_i<5) then {hint format ["Ziel %1 , Zeit abgelaufen", _i]}
            else {hint format ["Ziel %1 , Zeit abgelaufen", _i];sleep 2;hint "Übungsende, Übungsende!";};  
        }
        else
        {
            if (_object  getVariable ["target_range_abort_mg_zf", false]) exitWith {};
            if (_i<5) then {hint format ["Ziel %1 vernichtet", _i]}
    ...
    //if !(_object  getVariable ["target_range_isUp_mg_zf", false]) exitWith {};
    if (_object  getVariable ["target_range_abort_mg_zf", false]) exitWith {_target animate ["terc",1]};
