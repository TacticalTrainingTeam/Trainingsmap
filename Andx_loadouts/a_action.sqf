...
if (_selectedIndex == 19) exitWith {
	closeDialog 0;
	hint "Du bist nun Munitionsträger";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "green"]);
...
if (_selectedIndex == 29) exitWith {
	closeDialog 0;
	hint "Du bist nun Braun - Arzt (BAT)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
...
if (_selectedIndex == 39) exitWith {
	closeDialog 0;
	hint "Du bist nun Fire Direction Commander";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
...
if (_selectedIndex == 42) exitWith {
	closeDialog 0;
	hint "Du bist nun Zugführer (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_ZugF.sqf";
};

if (_selectedIndex == 43) exitWith {
	closeDialog 0;
	hint "Du bist nun Gruppenführer (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_GF.sqf";
};

if (_selectedIndex == 44) exitWith {
	closeDialog 0;
	hint "Du bist nun Truppführer (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_TF.sqf";
};

if (_selectedIndex == 45) exitWith {
	closeDialog 0;
	hint "Du bist nun Crew (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_Crew.sqf";
};

if (_selectedIndex == 46) exitWith {
	closeDialog 0;
	hint "Du bist nun Grenadier (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_Gren.sqf";
};

if (_selectedIndex == 47) exitWith {
	closeDialog 0;
	hint "Du bist nun LMG-Schütze (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_LMG.sqf";
};


if (_selectedIndex == 47) exitWith {
	closeDialog 0;
	hint "Du bist nun LAT-Schütze (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_LAT.sqf";
};


if (_selectedIndex == 49) exitWith {
	closeDialog 0;
	hint "Du bist nun AT-Schütze (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_AT.sqf";
};

if (_selectedIndex == 50) exitWith {
	closeDialog 0;
	hint "Du bist nun DMR-Schütze (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_DMR.sqf";
};


if (_selectedIndex == 51) exitWith {
	closeDialog 0;
	hint "Du bist nun Schütze (mech. Inf.)";
	private _helm = (player getVariable ["helm", "1"]);
	private _tarn = (player getVariable ["tarn", "BW_Flecktarn"]);
	private _farbe = (player getVariable ["farbe", "black"]);
	[player, _helm, _tarn, _farbe] spawn compile preprocessFileLineNumbers "loadouts\TTT_Schwarz_Rifle.sqf";
};
